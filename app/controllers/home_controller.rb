class HomeController < ApplicationController
  def index
   ...
   @notes_filter = Note.all.paginate(:per_page => 15, :page => params[:page]) 
   @labels = Intrest.where(:edunova_cert => 1)  	
  end
  def mark_as_read
  ...
  def activity
      ...
      if params[:search] 
        @notes_filter = Note.isearch(params[:search]).paginate(:per_page => 15, :page => params[:page])     
      else
        @notes_filter = Note.all.paginate(:per_page => 15, :page => params[:page]) 
      end
  end
